...
static void my_output_message(j_common_ptr cinfo) {
  char buffer[JMSG_LENGTH_MAX];
  (*cinfo->err->format_message)(cinfo, buffer);
  LOG_WARN("encode_ijg_jpeg16(...):%s", buffer);
}
...
DICOMSDL_CODEC_RESULT encode_ijg_jpeg16(imagecontainer *ic, char **data,
  ...
  switch (jmode) {
    ...
    case JPEG_BASELINE:  // baseline or
    case JPEG_EXTENDED:  // extended encoding do not support 16bit image
    default:
      snprintf(ic->info, ARGBUF_SIZE, "ijg_codec::encode_ijg_jpeg16(...):\n"
               "set_pixeldata(...) should handle this!!!");
...
DICOMSDL_CODEC_RESULT decode_ijg_jpeg16(char *src, int srclen,
                                        imagecontainer *ic) {
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    memcpy(pixelbuf, buffer[0], row_stride * 2);  // bpp = 2
    pixelbuf += ic->rowstep;
  }
