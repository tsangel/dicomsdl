...
static void my_output_message(j_common_ptr cinfo) {
  char buffer[JMSG_LENGTH_MAX];
  (*cinfo->err->format_message)(cinfo, buffer);
  LOG_WARN("encode_ijg_jpeg12(...):%s", buffer);
}
...
DICOMSDL_CODEC_RESULT encode_ijg_jpeg12(imagecontainer *ic, char **data,
  ...
  switch (jmode) {
    ...
    case JPEG_BASELINE:  // baseline encoding do not support 12bit image
    default:
      snprintf(ic->info, ARGBUF_SIZE, "ijg_codec::encode_ijg_jpeg12(...):\n"
...
DICOMSDL_CODEC_RESULT decode_ijg_jpeg12(char *src, int srclen,
                                        imagecontainer *ic) {
