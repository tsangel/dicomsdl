...
conv_result_t jisx0208_to_unicode(conv_stat_t *stat) {
  while (stat->mb_left > 0) {
    ...
    if (b1 < 0x80) {
      ...
      uint16_t uc = map_jisx0208_multibyte_to_unicode  //
          [map_jisx0208_multibyte_to_unicode[b1] + b2];
      if (uc == 0)
        return CONV_ILLEGAL_SEQUENCE;
...
conv_result_t unicode_to_jisx0208(conv_stat_t *stat) {
  if (stat->set_g0 == NULL) {
  ...
  while (stat->wc_left > 0) {
    ...
    pageindex = map_jisx0208_unicode_to_index[uc >> 8];
    if (pageindex == 0)
      return CONV_ILLEGAL_UNICODE;

    codeindex = pageindex + ((uc & 0xf0) >> 3);
    usebit = map_jisx0208_unicode_to_index[codeindex + 1];
    codeindex = map_jisx0208_unicode_to_index[codeindex];
    usebit >>= (15 - (uc & 0xf));
    if ((usebit & 1) == 0)
      return CONV_ILLEGAL_UNICODE;
    ...
    mb = map_jisx0208_index_to_multibyte[codeindex];
    if (stat->last_charset_g0_used != CHARSET::JISX0208) {
      stat->last_charset_g0_used = CHARSET::JISX0208;
